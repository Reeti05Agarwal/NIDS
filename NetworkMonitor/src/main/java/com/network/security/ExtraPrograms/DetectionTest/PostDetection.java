...
public class PostDetection {
    ...
    static boolean isAnomalous(Map<String, Object> data) {
        ...
        if (protocol == 17 && totalLength > 1000) {
            System.out.println("[ALERT] Possible UDP Flood attack from " + srcIp);
            return true;
        }
    ...
    static void analyzeStoredPackets() {
        try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
            ...
            while (rs.next()) {
                Map<String, Object> packetData = new HashMap<>();
                packetData.put("SRC_IP", rs.getString("SRC_IP"));
                packetData.put("DEST_IP", rs.getString("DEST_IP"));
                packetData.put("PROTOCOL_NAME", rs.getInt("PROTOCOL_NAME"));
                packetData.put("TOTAL_LENGTH", rs.getInt("TOTAL_LENGTH"));
                packetData.put("TTL", rs.getInt("TTL"));
    
                if (isAnomalous(packetData)) {
                    System.out.println("[ALERT] Anomaly detected in stored packet: " + packetData);
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
